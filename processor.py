...
def initialize_database():
    ...
    try:
        print(f"processor.py:28 [{datetime.now().isoformat()}] - Connecting to DB at {DB_HOSTNAME}")
        conn = psycopg2.connect(database=DB_NAME, user=DB_USER, password=DB_PASS, host=DB_HOSTNAME, port=5432)
        cursor = conn.cursor()
